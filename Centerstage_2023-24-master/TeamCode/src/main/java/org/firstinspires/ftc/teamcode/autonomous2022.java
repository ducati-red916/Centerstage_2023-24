...
//@Autonomous(name = "autonomous2022")
public class autonomous2022 extends LinearOpMode {
            ...
            while (opModeIsActive()) {
                ...
                for (Recognition recognition_item2 : recognitions) {
                    ...
                    if (recognition.getLabel().equals("C")) {
                        while (back_right_port_1.getCurrentPosition() < 42000) {
                            front_left_port_0.setPower(-0.2*flr);
                            front_right_port_2.setPower(0.2*frr);
                            back_left_port_3.setPower(-0.2*blr);
                            back_right_port_1.setPower(0.2*brr);
                            displayInfo(index);
                            if(Math.abs(back_left_port_3.getCurrentPosition())>50){
                                if (back_left_port_3.getCurrentPosition()>0){
                                    flr=1;
                                    frr=0.8;
                                    blr=1;
                                    brr=0.8;
                        ...
                        powerto0();
                        // moving right
                        while (back_left_port_3.getCurrentPosition() < 42000) {
                            front_left_port_0.setPower(-0.4);
                            front_right_port_2.setPower(0.4);
                            back_left_port_3.setPower(-0.4);
                            back_right_port_1.setPower(0.4);
                            displayInfo(index);
                        }
                        powerto0();
                        slide_down();
                    ...
                    if (recognition.getLabel().equals("S")) {
                        while (back_right_port_1.getCurrentPosition() < 47500) {
                            front_left_port_0.setPower(-0.2);
                            front_right_port_2.setPower(0.2);
                            back_left_port_3.setPower(-0.2);
                            back_right_port_1.setPower(0.2);
                            displayInfo(index);
                        }
                        powerto0();
                        // moving right
                        while (back_left_port_3.getCurrentPosition() > -40000) {
                            front_left_port_0.setPower(-0.4);
                            front_right_port_2.setPower(0.4);
                            back_left_port_3.setPower(-0.4);
                            back_right_port_1.setPower(0.4);
                    ...
                    if (recognition.getLabel().equals("T")) {
                        while (back_right_port_1.getCurrentPosition() < 50000) {
                            front_left_port_0.setPower(-0.2);
                            front_right_port_2.setPower(0.2);
                            back_left_port_3.setPower(-0.2);
                            back_right_port_1.setPower(0.2);
    ...
    private void displayInfo(int i) {
        ...
        telemetry.addData("OWBack Current POS", back_left_port_3.getCurrentPosition());
        telemetry.addData("OWLeft Current POS", back_right_port_1.getCurrentPosition());
